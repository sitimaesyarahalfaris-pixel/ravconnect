    ...
    <main class="flex-1 bg-gray-50 py-12 px-8">
        <div class="max-w-xl mx-auto">
            <div class="flex items-center justify-between mb-6">
                <h1 class="text-2xl font-black">Edit Country: {{ $country->name }}</h1>
                <div class="text-sm text-gray-600">Logged in as: {{ auth()->user()->name }}</div>
            </div>

            <form method="POST" action="{{ route('admin.countries.update', $country->id) }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label class="font-bold">Country Name</label>
                    <input type="text" name="name" class="w-full border rounded p-2" value="{{ $country->name }}" disabled>
                </div>
                <div class="mb-4">
                    ...
                    @if($country->image_url)
                        <div class="mb-2"><img src="{{ $country->image_url }}" alt="Header" class="max-w-xs rounded shadow"></div>
                    @endif
